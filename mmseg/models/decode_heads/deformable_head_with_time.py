...
def save_fusion_image(fusion_out, save_path='fusion_out_image.png'):
    """
    保存 fusion_out[0] 作为图像文件。
    
    参数:
    fusion_out (torch.Tensor): 形状为 [C, H, W] 的张量，表示要保存的图像。
    save_path (str): 保存图像的路径，默认为 'fusion_out_image.png'。
    """
    # 确保 fusion_out 是一个 CPU 张量
    for idx,fusion_out_ in enumerate(fusion_out):
        fusion_out_image = fusion_out_.cpu()  # 获取第一个图像张量并移至 CPU

        # 转换为 [H, W, C] 格式
        fusion_out_image = fusion_out_image.permute(0,1, 2)

        # 如果值范围是 [0, 1]，需要乘以 255 转换为 [0, 255] 范围
        fusion_out_image = (fusion_out_image * 255).clamp(0, 255).to(torch.uint8)

        # 转换为 PIL 图像
        pil_image = ToPILImage()(fusion_out_image)

        # 保存图像
        pil_image.save(f'fusion_out_image{idx}.png')
